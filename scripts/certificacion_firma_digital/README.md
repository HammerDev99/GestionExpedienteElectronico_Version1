...
| ~~_pendiente_~~ | ~~Cotización formal EV persona natural~~ — **descartado** por la decisión de que el área de Seguridad Informática firme con el certificado institucional (ver Fase 2) | — | — |
| ~~_pendiente_~~ | ~~Compra e instalación cert EV definitivo~~ — **descartado** por la misma decisión | — | — |
...
| 2026-09-05 | **Prueba manual del instalador MSI completada**: los diez pasos de la guía pasan correctamente, incluida la validación del `UpgradeCode` (instalar 1.5.2 sobre 1.5.1 deja una sola entrada en Agregar o quitar programas), que es la única decisión del paquete sin corrección remota posible tras su distribución. Dos defectos detectados y corregidos durante la prueba: el MSI se compilaba sin declarar arquitectura y Windows lo instalaba en `Program Files (x86)` por redirección, y la guía no fijaba el PATH de WiX, lo que hacía fallar silenciosamente la prueba del `UpgradeCode` | Daniel Arbeláez | `evidencias/validacion_msi.txt`, `installer/PRUEBA_MANUAL_MSI.md` |
| 2026-09-05 | Mitigación del congelamiento de la interfaz durante el procesamiento (Windows marcaba la ventana como "No responde" pese a que el proceso avanzaba). Comportamiento preexistente, ajeno al empaquetado MSI. Solución estructural documentada como trabajo pendiente | Daniel Arbeláez + Claude | `docs/superpowers/specs/2026-09-05-responsividad-gui-procesamiento.md` |
